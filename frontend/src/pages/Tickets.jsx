...
function Tickets() {
  ...
  //
  // IMPORTANT:
  // Do not use ticketId as a parent booking ID when a real
  // booking ID exists.
  // ============================================================
  ...
  // ============================================================
  // GET FALLBACK TICKET ID
  // ============================================================
  function getFirstTicketId(booking) {
    const tickets = getBookingTickets(booking);
    return tickets?.[0]?.ticketId || null;
  }
  // ============================================================
  // GET BOOKING TICKETS
  // ============================================================
        ...
        try {
          ...
          if (attendanceResponse.ok) {
            ...
            if (Array.isArray(attendanceData)) {
              allPasses = attendanceData;
            } else if (
              Array.isArray(
                attendanceData?.attendance
              )
            ) {
              allPasses = attendanceData.attendance;
            } else if (
              Array.isArray(attendanceData?.passes)
            ) {
              allPasses = attendanceData.passes;
            }
            myPasses = allPasses.filter(
              (pass) => {
                const passEmail =
                  pass?.email ||
                  pass?.buyer?.email ||
  ...
  const sortedBookings = useMemo(() => {
    return [...bookings].sort((a, b) => {
      const aPassed = hasEventPassed(
        getEventDate(a)
      );
      const bPassed = hasEventPassed(
        getEventDate(b)
      );
      if (aPassed !== bPassed) {
        return aPassed ? 1 : -1;
  ...
  // ============================================================
  const deletePastBooking = async (booking) => {
    const bookingTickets =
      getBookingTickets(booking);
    ...
    try {
      ...
      const deleteKey =
        bookingId ||
        getFirstTicketId(booking);
      setDeletingBookingId(deleteKey);
      // Delete each individual ticket.
      // The backend supports deletion by ticketId.
              ...
              (pass) => {
                ...
                return (
                  ...
                  >
                    ...
                    <div className="ticket-actions">
                      <Link
                        to={`/free-ticket/${encodeURIComponent(
                          pass.id ||
                            pass.attendanceId
                        )}`}
                        className="view-ticket-btn"
